...
public class PlasmaHostedService : IHostedService
{
    ...
    private static readonly TimeSpan SslHandshakeTimeout = TimeSpan.FromSeconds(10);
    ...
    private async Task HandleTcpConnection(TcpClient tcpClient, int connectionPort)
    {
        ...
        if (PortExtensions.IsTheater(connectionPort) || _arcadiaSettings.TheaterPorts.Contains(connectionPort))
        {
        ...
        else
        {
            ...
            try
            {
                ...
                {
                    // Crypto is per-connection (transient), so handshakes run concurrently;
                    // the blocking Accept borrows a pool thread bounded by the timeout below.
                    ...
                    var handshake = Task.Run(() =>
                    {
                        ...
                        {
                            _logger.LogDebug(e, "TLS handshake failed: {ep}", remoteEndpoint);
                            return false;
                        }
                    });
                    ...
                    try
                    {
                        handshakeOk = await handshake.WaitAsync(SslHandshakeTimeout, cts.Token);
                    }
                    catch (TimeoutException)
                    {
                        _logger.LogWarning("TLS handshake timed out from {ep}", remoteEndpoint);
                        handshakeOk = false;
                    }
                    ...
                    if (!handshakeOk)
                    {
                        // Releases an Accept still parked inside the abandoned handshake task.
                        try { networkStream.Dispose(); } catch { }
                        return;
                    }

                    // Same scoped instance the handler's IEAConnection resolves to; the raw
                    // stream lets Terminate unblock the read pump without a TLS close_notify.
                    scope.ServiceProvider.GetRequiredService<IEAConnection>().Transport = networkStream;
    ...
    {
        var isTheater = PortExtensions.IsTheater(port) || _arcadiaSettings.TheaterPorts.Contains(port);
        if (isTheater)
        {
